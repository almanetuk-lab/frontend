...
const INITIAL_FORM_STATE = {
  user_id: getUserFromStorage().id || null,
  profilePhotoUrl: "",
  fullName: getUserFromStorage().fullName || "",
  headline: "",
  age: "",
  gender: "",
  maritalStatus: "",
  ...
  phone: "",
  education: "",
  profession: getUserFromStorage().profession || "",
  company: "",
  experience: "",
  skills: "",
  about: "",
...
export default function EditProfile() {
  ...
  useEffect(() => {
    // Load profile from context or storage
    if (!authToken) {
  ...
  if (loading && !formData.user_id && !apiError) {
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {formData.interests.map(t => (
              <span key={t} className="inline-flex items-center bg-gray-100 px-3 py-1 rounded-full text-sm">
                {t} <button type="button" onClick={() => removeInterest(t)} className="ml-2 text-red-500 font-bold">×</button>
              </span>
            ))}
          </div>
        </div>
...
//     };
//     loadUserProfile();
//   }, [authToken, updateProfile, currentUserId]);
...
//       alert("Profile saved successfully!"); // ✅ Alert on success
//       navigate("/dashboard");
//     } catch (error) {
//       setApiError(error.message || "Failed to save profile.");
//       console.error("Error saving profile:", error);
